        ...
        <>
            <motion.header
                ...
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b border-transparent ${
                    scrolled || mobileOpen
                        ? "bg-[#021021]/5 backdrop-blur-2xl border-white/10"
                        : "bg-transparent"
                }`}
            >
                            ...
                            <div className="px-6 pb-6 pt-4">
                                {navItems.map((item) => (
                                    <button
                                        key={item.id}
                                        onClick={() => scrollTo(item.id)}
                                        className="block w-full text-left py-3 text-white/60 hover:text-white text-md sm:text-sm font-semibold border-b border-white/5 transition-colors"
                                    >
                                        {item.label}
                                    </button>
                                ))}
                                <button
                                    onClick={() => scrollTo("get-started")}
                                    className="mt-4 w-full text-md sm:text-sm px-5 py-2.5 rounded-full font-semibold"
                                    style={{
                                        background:
                                            "linear-gradient(135deg, #BAFF4E, #8FD920)",
                                        color: "#08080D",
                                    }}
                                >
                                    Download App
